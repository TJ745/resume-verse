...
// Always fetch fresh from DB — critical so plan status is correct after upgrade
...
export default async function DashboardPage() {
  ...
  // Fetch user plan alongside resumes in one query
    ...
    <div className="max-w-6xl mx-auto px-8 py-12">
      ...
      <div className="flex items-end justify-between mb-10 pb-8 border-b border-rv-border">
        <div>
          ...
          <h1 className="font-serif text-[2.25rem] leading-[1.1] tracking-[-0.025em]">
            Welcome back,{" "}
            <em className="italic text-rv-accent">{firstName}</em>
          </h1>
        </div>

        <div className="flex items-center gap-3">
          <UploadResumeButton isPro={isPro} />
          <span className="w-px h-5 bg-rv-border inline-block shrink-0" />
          <p className="text-sm text-rv-muted whitespace-nowrap">
            {resumes.length} {resumes.length === 1 ? "resume" : "resumes"}
          </p>
        ...
        <Suspense fallback={null}>
          {resumes.map((resume) => (
            <ResumeCard
              ...
              fontSize={(resume as typeof resume & { fontSize?: string }).fontSize ?? "md"}
              updatedAt={resume.updatedAt}
              jobTitle={resume.jobTitle ?? undefined}
            />
          ))}
      ...
      {resumes.length === 0 && (
        <div className="text-center mt-16">
          <p className="font-serif text-[1.5rem] text-rv-ink mb-2">No resumes yet</p>
          <p className="text-rv-muted text-[0.9375rem]">
            Create a new resume or upload an existing one to get started.
          </p>
        </div>
