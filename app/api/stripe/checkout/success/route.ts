...
// We use {CHECKOUT_SESSION_ID} so Stripe fills in the real session ID.
//
// This route:
//   1. Retrieves the Stripe checkout session by ID
//   2. Verifies it is paid and belongs to a known user (via metadata.userId)
//   3. Upgrades the user to Pro in the DB
//   4. Redirects to /dashboard?upgrade=success
//
// This is the authoritative upgrade path. The webhook handler is a
// redundant safety net for renewals and cancellations only.
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (session.payment_status !== "paid" && session.status !== "complete") {
      console.warn(`[checkout/success] Session ${sessionId} not paid: ${session.payment_status}`);
      return NextResponse.redirect(errorUrl);
    }
    ...
    if (!userId) {
      console.error(`[checkout/success] No userId in metadata for session ${sessionId}`);
      return NextResponse.redirect(errorUrl);
    }
    ...
    const rawPeriodEnd =
      (subscription?.items?.data?.[0] as Record<string, unknown>)?.current_period_end ??
